...
const Businessmap = () => {
    ...
    const accordionData = [
        ...
        { title: 'ব্যবসায়িক প্রস্তাব বা প্লান করুন', content: 'আপনার প্রস্তাব বা পরিকল্পনা তৈরি করুন।' },
        { title: 'ওয়েব সাইট ডিজাইন সিলেক্ট করুন', content: 'আপনার পছন্দসই ডিজাইন নির্বাচন করুন।' },
        { title: 'প্রথম পেমেন্ট করুন', content: 'আপনার প্রথম পেমেন্ট সম্পন্ন করুন।' }
    ];

       // ---------------all-courses--------------
const [accordions,set_accordions]=useState([]);
    const get_accordions=()=>{
        axios.get(`${base_url}/admin/all-accordions`)
        .then((res)=>{
            if(res.data.success){
...
const [footer_video,set_footer_video]=useState([]);
 ...
 <div className='flex justify-center items-center '>
     ...
     <div className="flex flex-col lg:flex-row gap-[20px] lg:gap-[60px] mt-[50px] ">
          ...
          {
            footer_video?   <div className="w-[100%] md:w-[80%] lg:w-[45%]">
       <div className="flex flex-col items-center justify-center lg:p-4">
            {/* Thumbnail with Play Icon */}
            <div className="relative cursor-pointer w-full bg-white p-[7px] rounded-[10px]" onClick={togglePopup}>
              {/* Thumbnail */}
              <img
                src={`${base_url}/images/${footer_video?.thumbnail}`}
                alt="Video Thumbnail"
                className="w-full h-[180px] md:h-[280px] lg:h-[280px] rounded-[10px] "
              />
            ...
            {isOpen && (
              <div className="fixed inset-0 z-[100000] bg-black bg-opacity-75 flex items-center justify-center ">
                ...
                <div className="w-full lg:w-1/2 font-bangla_font ">
                    <div className="grid grid-cols-1 gap-[8px] lg:gap-4">
                        {accordions?.map((item, index) => (
                            <div key={index} className="border font-poppins  border-gray-300 rounded-lg overflow-hidden shadow-sm">
                                <button
