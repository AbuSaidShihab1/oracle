...
const [video_reviews,set_video_reviews]=useState([]);
  ...
  return (
    <>
    {
      video_reviews.length > 0 ?   <section className='w-full bg-gradient-to-r from-blue-500 to-purple-500 h-auto px-[20px] md:px-[30px] lg:px-[50px] xl:px-[100px] py-[40px] lg:py-[70px] '>
        ...
        <section className='pt-[30px] lg:pt-[50px] grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-[20px]'>
  {
    video_reviews?.slice(0,9).map((data,i)=>{
     ...
     <div key={i} className='w-full overflow-hidden  rounded-[10px] p-[7px] bg-white'>
            <iframe className='w-full h-[180px] lg:h-[250px] rounded-[10px]'  src={`${data.video_link}`} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
          </div>
      )
    })
  }
           ...
           {/* -------------box----------------- */}
  {
    video_reviews?.length > 9 ?  <div className="flex w-full items-center justify-center mt-[50px] z-[100]">
       <NavLink to="/video-reviews">
              <button className='px-[30px] py-[12px] text-white rounded-[5px] bg-indigo-800 border-[2px] border-white font-poppins text-[16px]'>Load more</button>
       </NavLink>
         </div>:""
  }
